 ...
 * @param type  输入的GNSS数据的坐标系类型
 */
Gnss::Gnss(const RTK::SharedPtr &msg, CoordType type) {
    stamp_ = msg->header.stamp.sec + msg->header.stamp.nanosec * 1e-9;
    convert_success_ = ConvertGnss2Utm(msg->latitude, msg->longitude, msg->altitude, msg->yaw, msg->yaw_valid, type);
}
...
bool Gnss::ConvertGnss2Utm(double lat, double lon, double alt, double yaw, bool valid, CoordType type) {
    ...
    long ret = Convert_Geodetic_To_UTM(lat * M_PI / 180.0, lon * M_PI / 180.0, &zone, &hemisphere, &easting, &northing);
    if (ret != 0)
        return false;
